...
public class PRQT extends Quadtree {
		...
		public Node remove(Geometry2D a) {
			...
			for (int i = 0; i < 4; ++i) {
				...
				if (kids[i] instanceof PRQTWhite) {
					...
					++blackCount;
					blacksGeo = ((PRQTBlack) kids[i]).geos.first();
				}
			}
	...
	public TreeSet<PointGeometry2D> metropolesInRange(RangeCircle circ) {
		TreeSet<PointGeometry2D> res = new TreeSet<PointGeometry2D>(PointGeometry2D.getPointCoorComp());
		citiesInRangeHelper(circ, root, res);
		return res;
	}
	
	private void citiesInRangeHelper(RangeCircle circ, Node cur, TreeSet<PointGeometry2D> res) {
		if (cur instanceof Gray) {
			Gray qur = (Gray) cur;
			for (int i = 0; i < 4; ++i) {
				Node kid = qur.kids[i];
				if (Utilities.geosIntersect(kid, circ, true)) {
					citiesInRangeHelper(circ, kid, res);
				}
			}
		} else if (cur instanceof Black) {
			Black qur = (Black) cur;
			PointGeometry2D pg2d = qur.getPointGeometry2D();
			if (pg2d != null && Utilities.geosIntersect(pg2d, circ, false)) {
				res.add(pg2d);
			}
		}
	}
